...
```
```{r}
# Scatter plots to view our predictors of interest
ggplot(grapes, aes(x = sun_exposure_hours, y = sugar_content_brix, color = region)) +
  geom_point(alpha = .8) +
  geom_smooth(method = "ls", se = FALSE) +
  facet_wrap(~ region) +
  labs(title = "Sun Exposure vs Brix by Region",
...
```
```{r}
# This plot will provide scores by weekly breaks, not that July and October is included but no dates are present in data set
...
# Plot the graph based on only type of grape and a specified region
...
The Gamma regression model was able to retrieve the known parameters when tested.
- Multicollinearity: Multicollinearity was not detected by the lack of strong correlations
Independence: Predictors are independent 
